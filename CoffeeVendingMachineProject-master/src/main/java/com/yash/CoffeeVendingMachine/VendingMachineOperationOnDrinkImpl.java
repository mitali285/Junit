...
public class VendingMachineOperationOnDrinkImpl implements VendingMachineOperationsonDrinkInterface {

	Logger logger = Logger.getLogger(VendingMachineOperationOnDrinkImpl.class.getName());

	public VendingMachineOperationOnDrinkImpl() {
	...
	private Map<String, Integer> drinkTypeAndQuantity = new HashMap<String, Integer>();
	private Map<String, Double> drinkTypeAndPrice = new HashMap<String, Double>();
	...
	public Map<String, Double> checkTotalTeaCoffeeSaleAndWasteMaterialReport(Map<String, Integer> report) {
		...
		Map<String, Double> totalTeaCoffeeSaleReportmap = new HashMap<String, Double>();
		Map<String, Double> drinkWisePrice = new HashMap<String, Double>();
		...
		if (report.containsKey("BLACK_COFFEE")) {
			...
			totalTeaCoffeeSaleReportmap.put(str, report.get("BLACK_COFFEE") * DrinksType.BLACK_COFFEE.getPrice());
	...
	public Integer refillContainer(Container container, Integer choice, Integer amount) {
		...
		case 1:
			if (amount <= 2000 - container.getTeaContainer()) {
				container.setTeaContainer(container.getTeaContainer() + amount);
				refillCounter += 1;
				isRefilled = true;
			} else {
		...
		case 2:
			if (amount <= 2000 - container.getCoffeeContainer()) {
				container.setCoffeeContainer(container.getCoffeeContainer() + amount);
				refillCounter += 1;
				isRefilled = true;
			} else {
		...
		case 3:
			if (amount <= 10000 - container.getMilkContainer()) {
				container.setMilkContainer(container.getMilkContainer() + amount);
		...
		case 4:
			if (amount <= 8000 - container.getSugarContainer()) {
				container.setSugarContainer(container.getSugarContainer() + amount);
